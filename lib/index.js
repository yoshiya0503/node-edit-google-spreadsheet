...
Spreadsheet.prototype.request = function(opts, callback) {
  ...
  request(opts, function(err, response, body) {
    ...
    // 1. Try to parse XML
    // 2. If invalid, assume it's JSON and parse it
    ...
    function send(next) {
      _this.request({
        ...
        try {
          ...
          var entriesNoSuccess = _.filter(entries, function(e) {
            return e['batch:status'].code !== 200;
          });

          if (entriesNoSuccess.length) {
            _this.log("Error while updating spreadsheet:");
            _.each(entriesNoSuccess, function(e) {
              _this.log("entry: " + status.code + " > " + status.reason);
            });

            next("Your update may not fit in the worksheet. See the 'autoSize' option");
          } else {
